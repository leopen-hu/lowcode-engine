    ...
    if (strictMode) {
      ...
      Object.keys(engineOptions).forEach((key) => {
        if (isValidKey(key)) {
          ...
          logger.warn(
            `failed to config ${key} to designerConfig, only predefined options can be set under strict mode, predefined options: `,
...
export const designerConfig = new DesignerConfig();
